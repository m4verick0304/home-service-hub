...
const HelperLogin = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col lg:flex-row relative">
      {/* Full-screen Silk background */}
      <div className="absolute inset-0 z-0 bg-background/90">
        <Silk speed={5} scale={1} color="#10b981" noiseIntensity={1.5} rotation={0} />
      </div>
        ...
        <div className="relative flex flex-col justify-between h-full p-8 lg:p-12">
          <div className="flex items-center gap-2">
            <button onClick={() => navigate("/")} className="flex items-center gap-2 hover:opacity-80 transition-opacity">
              <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-white/15 text-white text-[10px] font-bold tracking-tight">US</div>
              <span className="text-white text-base font-semibold tracking-tight">UrbanSquare</span>
            </button>
          ...
          <div>
            ...
            <p className="mt-3 text-white/70 text-sm lg:text-base max-w-md">
              Join thousands of skilled professionals earning with UrbanSquare. Accept jobs, build your reputation, grow your income.
            </p>
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-12 lg:py-0 z-10">
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="w-full max-w-sm bg-background/80 backdrop-blur-xl rounded-2xl p-8 shadow-2xl border border-border/50">
          ...
          <p className="mt-6 text-center text-sm text-muted-foreground">
            {isSignUp ? "Already a helper?" : "New to UrbanSquare?"}{" "}
