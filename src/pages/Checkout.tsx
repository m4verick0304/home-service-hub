...
const Checkout = () => {
  ...
  const handlePlaceOrder = async () => {
    ...
    // For card, open payment form (handled in UI). For other methods, finalize immediately.
    if (paymentMethod === "card") return; // Card flow handled by PaymentForm
      ...
      <motion.main initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="mx-auto max-w-2xl px-4 sm:px-6 py-6 space-y-6">
        ...
        {paymentMethod === "card" && (
          <div className="p-4 rounded-2xl bg-card border sh-shadow">
            <Elements stripe={loadStripe(String(import.meta.env.VITE_STRIPE_PUBLISHABLE_KEY))}>
              <PaymentForm amount={grandTotal} onSuccess={async (paymentId) => await finalizeBooking(paymentId)} />
            </Elements>
          </div>
        )}
